...
local toggle_highlights = function()
  ...
  local existing_extmarks = vim.api.nvim_buf_get_extmarks(buf, namespace, {
    start_line,
    0,
   }, {
    end_line,
  ...
  for line = start_line, start_line + lines_range, 1 do
    local existing_extmark_id =
        get_extmark_id_if_exists(line, existing_extmarks)
...
-- Create autocommands
vim.api.nvim_create_user_command("HighlightToggle",
                                 function() return toggle_highlights() end, {
  nargs = "?",
 })
...
vim.keymap.set({
  "n",
  "v",
 }, "<leader>hs", function() vim.cmd(":HighlightToggle") end, {
  silent = true,
 })
vim.keymap.set({
  "n",
  "v",
 }, "<leader>hd", function() wipe_extmarks(namespace) end, {
  silent = true,
 })
